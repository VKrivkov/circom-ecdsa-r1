...
template Secp256k1PrimeReduce10Registers() {
    ...
    signal output out[4];
    var offset1 = (1 << 32) + (1 << 16) - 1; 
    var offset2 = (1 << 128) + (1 << 64); 

    out[3] <== (offset2 * in[7]) + in[3];
    out[2] <== (offset1 * in[6]) + in[2] + in[9];
    out[1] <== (offset2 * in[9]) + (offset1 * in[5]) + in[1] + in[8];
    out[0] <== (offset2 * in[8]) + (offset1 * in[4]) + in[0];
}
...
template CheckCubicModPIsZero(m) {
    ...
    for (var i = 0; i < 4; i++) {
        multipleOfP[i] <== p[i] * (1 << (m - 20)); // Adding 2^(m-20) times p[i]
    }
    for (var i = 0; i < 4; i++) {
